...
# This file is generated by pp. do not edit.
# source is anvalible at: ./src/YN_confirm.sh
YN_confirm() {

local io_file

case ${YN_CONFIRM_TTY_PATH+x} in
	x) io_file=$YN_CONFIRM_TTY_PATH;;
	*)
		io_file=/dev/tty;
		[ -w "${io_fil}" ] || {
...
while case $# in 0) false;; *) ;;esac; do
	case $1 in
		...
		--stdin|-S)
			case ${YN_CONFIRM_TTY_PATH+x} in
				x) printf %s\\n >&2 "${FUNCNAME:-YN_confirm}: Ignoring -S option because YN_CONFIRM_TTY_PATH var is in the env.";;
				*) unset io_file
			esac
		;;
		[+-]x) set "$1";;
